...
class Owners extends DataSource
{
    public $db_link;
    public $db_link_flag;
        ...
        $this->query->where('AND', function (& $query) use ($parameters) {
            ...
            $query->where('OR', function (& $query) use ($parameters) {
                ...
                $query->where('AND', function (& $query) use ($parameters) {
                    $query = Types::queryBuilderStatic(['alias' => 'inner_w1'])->select();
                    ...
                    $query->join('INNER', new Organizations(), 'inner_w3', 'ON', [
                    ...
                    } elseif (!empty($parameters['owner_type_id'])) {
                        $query->where('AND', ['inner_w1.um_ownertype_id', '=', $parameters['owner_type_id']]);
                    }
                }, 'EXISTS');
                // roles are setup
                $query->where('AND', function (& $query) use ($parameters) {
                    $query = Types::queryBuilderStatic(['alias' => 'inner_v1'])->select();
                    ...
                    $query->join('INNER', new Roles(), 'inner_v3', 'ON', [
