...
class Notifications extends \Object\DataSource {
	public $db_link;
	public $db_link_flag;
	public $pk = ['user_id'];
	public $columns;
	...
	public function query($parameters, $options = []) {
		...
		$this->query->union('UNION', function (& $query) use ($parameters) {
			$query = \Numbers\Users\Users\Model\User\Roles::queryBuilderStatic(['alias' => 'b'])->select();
			$query->columns([
				'b.um_usrrol_user_id',
			]);
		...
		$this->query->union('UNION', function (& $query) use ($parameters) {
			$query = \Numbers\Users\Users\Model\Team\Map::queryBuilderStatic(['alias' => 'c'])->select();
			$query->columns([
		...
		$this->query->union('UNION', function (& $query) use ($parameters) {
			$query2 = \Object\Query\Builder::quick()->withRecursive('temp_rol_env_1000', ['id', 'parent_id'], function(& $query) use ($parameters) {
				$query = \Numbers\Users\Users\Model\Role\Children::queryBuilderStatic(['skip_acl' => true, 'alias' => 'inner_a'])->select();
				$query->columns([
				...
				$query->union('UNION ALL', function(& $query2) {
					$query2 =  \Numbers\Users\Users\Model\Role\Children::queryBuilderStatic(['skip_acl' => true, 'alias' => 'inner_c'])->select();
					$query2->columns([
			...
			$query->join('LEFT', new \Numbers\Users\Users\Model\User\Roles(), 'inner_e', 'ON', [
