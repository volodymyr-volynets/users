...
class Allocations extends Table
{
    ...
    public $constraints = [
        ...
        'um_ttallocation_on_organization_id_fk' => [
            ...
            'foreign_model' => Organizations::class,
    ...
    {
        $query = Allocations::queryBuilderStatic(['alias' => 'a'])->select();
        $query->columns([
        ...
        if ($result['success']) {
            foreach ($result['rows'] as $k => $v) {
                foreach ($v as $k2 => $v2) {
                    if ($v2['is_tracked']) {
                        $result['rows'][$k][$k2]['unfinished_hours'] = round(Date::diff($v2['last_start'], \Format::now('datetime'), 'abs hours', false), 2);
