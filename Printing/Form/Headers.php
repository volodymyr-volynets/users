...
class Headers extends \Object\Form\Wrapper\Base {
		...
		foreach ($form->values['\Numbers\Users\Printing\Model\Header\Fields'] as $k => $v) {
			...
			if (!empty($v['p8_hdrfield_other_options'])) {
				if (!is_json($v['p8_hdrfield_other_options'])) {
					$form->error(DANGER, \Object\Content\Messages::INVALID_VALUES, "'\Numbers\Users\Printing\Model\Header\Fields[{$k}][p8_hdrfield_other_options]");
				} else {
