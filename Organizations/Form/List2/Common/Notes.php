...
use Numbers\Users\Organizations\Model\Common\Note\Locations;
use Numbers\Users\Organizations\Model\Common\Note\Organizations;
use Object\Form\Wrapper\List2;

class Notes extends List2
{
    ...
    public function listQuery(& $form)
    {
        ...
        $form->query->join('LEFT', function (& $query) {
            $query = Locations::queryBuilderStatic(['alias' => 'inner_a'])->select();
            $query->columns([
        ...
        $form->query->join('LEFT', function (& $query) {
            $query = Organizations::queryBuilderStatic(['alias' => 'inner_b'])->select();
            $query->columns([
        ...
        if (!empty($form->values['locations_filter'])) {
            ...
            $form->query->where('AND', function (& $query) use ($locations_filter) {
                $query = Locations::queryBuilderStatic(['alias' => 'inner_c'])->select();
                $query->columns(1);
