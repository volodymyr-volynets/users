...
class Territories extends Base
{
    ...
    public $elements = [
        ...
        'general_container' => [
            'on_territory_node_type_id' => [
                ...
                'on_territory_organization_id' => ['order' => 3, 'label_name' => 'Organization', 'domain' => 'organization_id', 'null' => true, 'required' => true, 'percent' => 50, 'method' => 'select', 'tree' => true, 'options_model' => '\Numbers\Users\Organizations\Model\Organizations::optionsGroupedActive', 'onchange' => 'this.form.submit();'],
            ],
            'on_territory_parent_territory_id' => [
                'on_territory_parent_territory_id' => ['order' => 1, 'row_order' => 200, 'label_name' => 'Parent Territory', 'domain' => 'territory_id', 'null' => true, 'percent' => 100, 'method' => 'select', 'tree' => true, 'options_model' => '\Numbers\Users\Organizations\Model\Location\Territories::optionsGroupedActive', 'options_depends' => ['on_territory_type_id' => 'on_territory_type_id', 'on_territory_organization_id' => 'on_territory_organization_id'], 'options_params' => ['on_territory_node_type_id' => [10, 20]]],
            ],
    ...
    public function validate(& $form)
    {
        ...
        if ($form->values['on_territory_node_type_id'] == 30) {
            if (empty($form->values['on_territory_province_code'])) {
                $form->error(DANGER, Messages::REQUIRED_FIELD, 'on_territory_province_code');
            }
            // postal codes type
            if ($form->values['on_territory_type_id'] == 13) {
                if (empty($form->values['on_territory_postal_codes'])) {
                    $form->error(DANGER, Messages::REQUIRED_FIELD, 'on_territory_postal_codes');
                } else {
            ...
            if (empty($form->values['\Numbers\Users\Organizations\Model\Location\Territory\Locations'])) {
                $form->error(DANGER, Messages::REQUIRED_FIELD, '\Numbers\Users\Organizations\Model\Location\Territory\Locations[1][on_terrloc_location_id]');
            }
        }
        // primary location
