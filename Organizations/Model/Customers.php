...
class Customers extends Table
{
    ...
    public function optionsJson($options = [])
    {
        ...
        $organizations = Organizations::optionsStatic(['skip_acl' => true]);
        ...
        foreach ($data as $k => $v) {
            ...
            $parent = Options::optionJsonFormatKey(['organization_id' => (int) $v['organization_id'], 'customer_id' => null]);
            $this->optionsJsonReccursive($result, $organizations, $v['organization_id'], $parent);
            // actual customer
            $key = Options::optionJsonFormatKey(['organization_id' => (int) $v['organization_id'], 'customer_id' => (int) $k]);
    ...
    {
        if (!empty($result[$parent_key])) {
            return;
        }
        if (!isset($data[(int) $parent_id])) {
            return;
        }
        $result[$parent_key] = $data[(int) $parent_id];
        $result[$parent_key]['disabled'] = true;
        if (!empty($result[$parent_key]['parent_id'])) {
            $result[$parent_key]['parent'] = Options::optionJsonFormatKey(['organization_id' => (int) $result[$parent_key]['parent_id'], 'customer_id' => null]);
    ...
    public function optionsGrouppedTree($options = [])
    {
        ...
        $result = [];
        $organizations = Organizations::optionsStatic();
        foreach ($data as $k => $v) {
