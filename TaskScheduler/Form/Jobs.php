...
use Numbers\Users\TaskScheduler\Helper\Expression;
use Numbers\Users\TaskScheduler\Model\TaskParameters;
...
class Jobs extends Base
{
    ...
    public function validate(& $form)
    {
        ...
        if (!empty($form->values['ts_job_cron_minute']) && !$expression_model->isValidOneExpression($form->values['ts_job_cron_minute'])) {
            $form->error(DANGER, Messages::INVALID_VALUES, 'ts_job_cron_minute');
        }
        if (!empty($form->values['ts_job_cron_hour']) && !$expression_model->isValidOneExpression($form->values['ts_job_cron_hour'])) {
            $form->error(DANGER, Messages::INVALID_VALUES, 'ts_job_cron_hour');
        }
        if (!empty($form->values['ts_job_cron_day_of_month']) && !$expression_model->isValidOneExpression($form->values['ts_job_cron_day_of_month'])) {
            $form->error(DANGER, Messages::INVALID_VALUES, 'ts_job_cron_day_of_month');
        }
        if (!empty($form->values['ts_job_cron_month']) && !$expression_model->isValidOneExpression($form->values['ts_job_cron_month'])) {
            $form->error(DANGER, Messages::INVALID_VALUES, 'ts_job_cron_month');
        }
        if (!empty($form->values['ts_job_cron_day_of_week']) && !$expression_model->isValidOneExpression($form->values['ts_job_cron_day_of_week'])) {
            $form->error(DANGER, Messages::INVALID_VALUES, 'ts_job_cron_day_of_week');
        }
        if (!empty($form->values['ts_job_cron_year']) && !$expression_model->isValidOneExpression($form->values['ts_job_cron_year'])) {
            $form->error(DANGER, Messages::INVALID_VALUES, 'ts_job_cron_year');
        }
        ...
        foreach ($form->values['\Numbers\Users\TaskScheduler\Model\JobParameters'] as $k => $v) {
            if (!empty($mandatory_options[$v['ts_jbparam_name']])) {
                if (empty($v['ts_jbparam_value'])) {
                    $form->error(DANGER, Messages::REQUIRED_FIELD, "\Numbers\Users\TaskScheduler\Model\JobParameters[$k][ts_jbparam_value]");
                }
