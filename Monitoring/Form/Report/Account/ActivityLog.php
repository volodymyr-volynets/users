...
use Numbers\Users\Monitoring\Model\Usage\Actions;
...
class ActivityLog extends Report
{
    ...
    public function buildReport(& $form)
    {
        ...
        // query the data
        $query = Usages::queryBuilderStatic()->select();
        $query->columns('a.*');
        // join to get resource name
        $query->join('LEFT', new Resources(), 'b', 'ON', [
        ...
        $query->join('LEFT', new Users(), 'c', 'ON', [
        ...
        $query2 = Actions::queryBuilderStatic()->select();
        $query2->columns('a.*');
        ...
        // preload models
        $usage_codes = UsageCodes::optionsStatic(['i18n' => true]);
        // build report
        $counter = 1;
        foreach ($data['rows'] as $k => $v) {
            // replaces
            ...
            if (!empty($data2['rows'][$v['sm_monusage_id']])) {
                ...
                foreach ($data2['rows'][$v['sm_monusage_id']] as $k2 => $v2) {
                    $v2['sm_monusgact_url'] = ['value' => i18n(null, Messages::CLICK_HERE), 'url' => $v2['sm_monusgact_url']];
