...
import { Avatar, Initials } from '/src/Numbers/Avatar.jsx';
...
const RenderHtmlOnce = ({ html, className, key }) => {
  ...
  useEffect(() => {
    ...
    DOMPurify.addHook('uponSanitizeAttribute', function (currentNode, hookEvent) {
      // Only allow 'onclick' on button tags
      if (hookEvent.attrName === 'onclick' && currentNode.tagName === 'A') {
          hookEvent.forceKeepAttr = true; // Tell DOMPurify to preserve this attribute
      }
    });
    ref.current.innerHTML = DOMPurify.sanitize(html, {
      FORBID_TAGS: ['iframe', 'object', 'embed', 'form'], // 'style', 'script',
      FORBID_ATTR: ['onerror', 'onload', 'onmouseover', 'onfocus', 'onblur'], // 'style', 'onclick'
...
const ChatPageRightRail = ({
  ...
  const {
    ...
    // context
    __run_uuid: RunUUID,
    __context: ContextMainModelData,
    // can
    acl_can: AclCan,
    // dates
  ...
  const [needApprovalMessage, setNeedApprovalMessage] = useState(null);
  ...
  /** custom right rail phase form event listener */
  useEffect(() => {
    const handleRRPhaseFormEvent = (event: any) => {
      //alert(JSON.stringify(event.detail));
      setData({
        content: '<p>Form ' + event.detail['ai_tool_name'] + '</p>'
          + ' <p style="display: none;">Instructions:'
          + '<br/>call ' + event.detail['ai_tool_tool_name']
          + '<br/>context/params:<br/>' + event.detail['context'] + '</p>',
      });
    };
    window.addEventListener('wg_phase_form_rr_chat_event', handleRRPhaseFormEvent);
    return () => window.removeEventListener('wg_phase_form_rr_chat_event', handleRRPhaseFormEvent);
  }, []);
  ...
  const queryForChatChannels = async () => {
    ...
    if (result?.success) {
      ...
      // see if we require approval
      if (typeof ContextMainModelData['main_model_name'] !== 'undefined') {
        const is_previously_declined = sessionStorage.getItem('c5-chat-right-rail-declines-' + ChatID + '-' + ContextMainModelData['main_model_pk_key_string']);
        if (is_previously_declined != '1') {
          let full_context_found = false;
          for (let k in ContextMainModelData['main_model_pk']) {
            for (let j in result.context) {
              if (result.context[j]['tm_batchrecord_field_code'] == k && result.context[j]['tm_batchrecord_field_value_id'] == ContextMainModelData['main_model_pk'][k]) {
                full_context_found = true;
              }
            }
          }
          if (full_context_found) {
            setNeedApproval(false);
          } else {
            setNeedApproval(true);
            setNeedApprovalMessage(ContextMainModelData['main_model_message']);
          }
        }
      }
  ...
  const openSubFormTool = (value: any, force: boolean) => {
    ...
    if (saved != '1' || force) {
      window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', value['c5_chatmessage_form_settings_json']['link'], Object.assign({
        c5_chat_id: ChatID,
        c5_chat_current_um_user_id: ChatUserID,
        c5_chatmessage_id: value['c5_chatmessage_id'],
      }, value['c5_chatmessage_form_settings_json']['arguments']));
  ...
  const renderConversationRowsData = (data, is_thread) => {
    ...
    let first_td_width = is_thread ? '6%' : '50';
    let second_td_width = is_thread ? '94%' : '85%';
    let prefix = is_thread ? 'thrm' : 'cm';
        ...
        <>
          ...
          <li key={prefix + '-' + value['c5_chatmessage_id'] + '-' + value['hash']}>
            ...
            >
              ...
              <tr>
                <td rowspan="2" width={first_td_width} valign="top" align="center" style={{width: "50px"}}>
                  {!!value['c5_chatuser_photo_file_url'] && (
                    <ImgPhoto
                      src={value['c5_chatuser_photo_file_url']}
                      alt={Initials(value['c5_chatuser_um_user_name'])}
  ...
  const handleNewAIChatWindowClick = (type) => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_right_rail_new_chat_form', {
      c5_chatstarttype_code: type,
      main_model_pk: JSON.stringify(ContextMainModelData['main_model_pk'] ?? {}),
      main_model_model: ContextMainModelData['main_model_model'],
      main_model_name: ContextMainModelData['main_model_name'],
    });
  };

  const handleNewChannelChatWindowClick = (type) => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_new_message_on_channel_form', {
      c5_chatdmtype_code: type,
    });
  };

  const handleNewChannelWindowClick = () => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_new_channel_form', {});
  };

  const handleNewGroupWindowClick = () => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_new_group_form', {});
  };

  const handleViewGroupsWindowClick = () => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_groups_list', {});
  };

  const handleNewDMWindowClick = () => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_new_dm_form', {});
  };

  const handleViewCanvasesWindowClick = () => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_canvases_list', {});
  }

  const handleNewCanvasWindowClick = (type) => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_new_canvas_form', {
      c5_chatcanvas_c5_canvastype_code: type,
    });
  }

  const handleChangeAgentWindowClick = (c5_chat_id) => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_change_agent_form', {
      c5_chat_id: c5_chat_id,
    });
  }

  const handleChangeAgentThreadWindowClick = (c5_chat_id, c5_chatmessage_id) => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_change_thread_agent_form', {
      c5_chat_id: c5_chat_id,
      c5_chatmessage_id: c5_chatmessage_id,
    });
  }
  ...
  const handleChangeSignatureWindowClick = (c5_chat_id) => {
    ...
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_change_signature_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
  ...
  const handleChangeTermsWindowClick = (c5_chat_id) => {
    ...
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_change_terms_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
  ...
  const handleChangePromptsWindowClick = (c5_chat_id) => {
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_right_rail_add_prompt_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
  ...
  const handleChangeImageWindowClick = (c5_chat_id) => {
    const saved = JSON.parse(localStorage.getItem('nf_chat_image_c5_chatmessage_image_settings_json') ?? '{}');
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_add_image_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
  ...
  const handleChangeSoundWindowClick = (c5_chat_id) => {
    const saved = JSON.parse(localStorage.getItem('nf_chat_sound_c5_chatmessage_sound_settings_json') ?? '{}');
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_add_sound_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
      ai_agent_code: saved['ai_agent_code'] ?? null,
  ...
  const handleChangeTranscriptWindowClick = (c5_chat_id) => {
    const saved = JSON.parse(localStorage.getItem('nf_chat_transcript_c5_chatmessage_transcript_settings_json') ?? '{}');
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_add_transcript_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
      ai_agent_code: saved['ai_agent_code'] ?? null,
    });
  ...
  const handleChangeRAGWindowClick = (c5_chat_id) => {
    const saved = JSON.parse(localStorage.getItem('nf_chat_rag_c5_chatmessage_rag_settings_json') ?? '{}');
    window.Numbers.Form.openSubformWindow('', '', 'c5_chat_right_rail_form', 'c5_chat_standalone_add_rag_form', {
      c5_chat_id: c5_chat_id,
      um_user_id: ChatUserID,
            ...
            <td align="left" width="75%">
              ...
              {!!chatContextData && (
                <>
                  ...
                  <ThreadAddedA href={void(0)} onClick={() => handleNewAIChatWindowClick('AI')}>
                    {loc('NF.Form.New', 'New')}
                  </ThreadAddedA>
                </>
              )}
  ...
  return (
    <>
      <Row className="tw:gx-0 tw:gy-1 tw:text-left tw:divide-x-1 tw:divide-gray-200">
        {ChatUUID ? (
          <>
            <Col key={2} lg={12} md={12} ref={entireChatRef} className="tw:gx-0 tw:gy-1 tw:text-left tw:divide-y-1 tw:divide-gray-200">
              {/*
              <table width="100%">
                <tr>
              ...
              */}
              ...
              <div style={{position: 'relative'}}>
                <Row ref={conversationListHolderRef} style={{height: conversationListHolderHeight + 'px'}}>
                  <Col ref={conversationRowsRef} key={3} lg={12} md={12} sm={12} style={{overflowY: 'scroll', height: '100%'}}>
                    {conversationLastMessageID && renderConversationRowsData(conversationRowsData, false)}
                  </Col>
                </Row>
                {needApproval && (
                  <Row style={{position: 'absolute', top: (conversationListHolderHeight - 100) + 'px', height: '100px', width: '100%'}}>
                    <Col key={'3.5_approval'} lg={12} md={12} sm={12}>
                      <ApprovalTable width="100%">
                        <tr>
                          <td width="65%">
                            <RenderHtmlOnce
                              html={needApprovalMessage}
                              className="nf_chat_aproval_container"
                              key={'cm-approval-container'}
                            />
                          </td>
                          <td width="35%" align="right">
                            <div class="d-grid gap-2 mx-auto">
                              <button className="btn btn-primary" type="button" onClick={() => {}}>
                                {loc('NF.Form.Approve', 'Approve')}
                              </button>
                              <button className="btn btn-danger" type="button" onClick={() => {
                                setNeedApproval(false);
                                sessionStorage.setItem('c5-chat-right-rail-declines-' + ChatID + '-' + ContextMainModelData['main_model_pk_key_string'], '1');
                              }}>
                                {loc('NF.Form.Decline', 'Decline')}
                              </button>
                              <button className="btn btn-info" type="button" onClick={() => handleNewAIChatWindowClick('AI')}>
                                {loc('NF.Form.NewChat', 'New Chat')}
                              </button>
                            </div>
                          </td>
                        </tr>
...
export default ChatPageRightRail;
